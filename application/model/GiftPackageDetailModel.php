...
class GiftPackageDetailModel extends BaseModel
{
    ...
    public function detailedList($params,$user_id,$listRows){
        $EARTH=6378.137; //地球半径
        ...
        $coupons = $coupons->field("c.id,c.name,c.logo,s.name store_name,c.stock,c.original_price,c.buying_price,c.rebate_commission,c.promotion_commission,d.num,c.type,c.pattern{$field_distance}")
            ->paginate($listRows)
            ->each(function ($item){
